...
| 2.4 | 前端路由守卫 + token 存储 + Axios 拦截器（401 跳登录） | ▶ | | token 存储 + 拦截器已完；**路由守卫待补**（未登录访问 /admin 拦截） |

## 里程碑 3：商品域（product-service + 前台/后台页面）

| # | 任务 | 状态 | 完成日期 | 说明 |
|---|---|---|---|---|
| 3.1 | 类目 CRUD 接口 + 管理后台页面 | □ | | |
| 3.2 | 商品/SKU CRUD 接口 + 商家商品管理页（上架/下架） | □ | | |
| 3.3 | 广告位 CRUD + 管理后台页面 | □ | | |
| 3.4 | 前台首页（轮播广告+商品推荐）、商品列表（分类/搜索/分页）、商品详情（SKU 选择） | □ | | |
...
| 4.1 | 入驻审核 API + 后台审核页 | □ | | |
| 4.2 | 店铺信息管理 API + 商家页 | □ | | |
| 4.3 | 收款码/银行卡绑定 API + 页面 | □ | | |
| 4.4 | 提现申请/审核 API（balance 校验：申请减、驳回加）+ 提现日志页 | □ | | |

## 里程碑 5：交易域（order-service）

| # | 任务 | 状态 | 完成日期 | 说明 |
|---|---|---|---|---|
| 5.1 | 购物车 API（本地 localStorage 游客车 + 登录合并 + cart 表读改写） | □ | | |
| 5.2 | 下单 API（跨店拆单、服务端算价、Redis 预扣 + 同步条件扣库存） | □ | | |
| 5.3 | 我的订单/取消订单/确认收货/物流查看 + 前台页面 | □ | | |
| 5.4 | 商家发货 API + 商家订单管理页 | □ | | |
| 5.5 | 后台订单管理（全局查看/物流发货） | □ | | |
| 5.6 | RocketMQ 延迟消息：订单 30 分钟超时取消 + 库存回补 | □ | | |
